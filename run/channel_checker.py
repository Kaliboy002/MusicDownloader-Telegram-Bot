...
async def is_user_in_channel(user_id: int, channel_usernames: list = None) -> list:
    ...
    for channel_username in channel_usernames:
        ...
        while True:
            ...
            if not any(participant.id == user_id for participant in participants.users):
                ...
                break  # User not found, stop checking this channel

            offset += len(participants.users)  # Increment offset for the next batch
    return channels_user_is_not_in


def join_channel_button(channel_username: str) -> Button:
    """
...
async def respond_based_on_channel_membership(event, message_if_in_channels: str = None, buttons=None,
